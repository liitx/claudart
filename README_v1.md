...
2. Assigns them stable abstract tokens: `VolumeBloc` → `Bloc:A`, `RoverRepository` → `Repository:A`
...
The token map lives locally. It never leaves your machine. Anthropic builds understanding of `Bloc:A` across sessions without ever learning it's called `VolumeBloc`.
...
Once `VolumeBloc` → `Bloc:A`, that mapping is permanent. If the class is renamed, the old token gets a `renamedTo` field. If deleted, `deprecated: true`. The token is never reused for a different class. This means Anthropic's understanding of `Bloc:A` accumulates correctly across every session.
